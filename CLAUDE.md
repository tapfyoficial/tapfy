...
Audio has a "startFrom" prop that trims the left side of an audio by a number of frames.
Audio has a "endAt" prop that limits how long audio is shown.
...
The "Series.Sequence" component works like "Sequence", but has no "from" prop. Instead, it has a "offset" prop that shifts the start by a number of frames.
...
"TransitionSeries.Sequence" works like "Series.Sequence" but has no "offset" prop. The order of tags is important: "TransitionSeries.Transition" must be between "TransitionSeries.Sequence" tags.
